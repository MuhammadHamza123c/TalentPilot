...
from .prompt import prompt_interview
...
#(-----------------------------------INTERVIEW AGENT-------------)
def ask_questions(image_name:str,user_text:str):
    try:
        res_text=get_data(image_name=image_name)
        messages = [
            {
                "role": "system",
                "content": prompt_interview + f"\n\nHere is the candidate resume:\n{res_text}"
            }
        ]
        ...
        model=model_data[2]['INTERVIEW_MODEL'],
        temperature=model_data[2]['TEMPERATURE']
    )
